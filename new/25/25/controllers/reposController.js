...
class ReposController {
    async getAllRepos(req, res) {
        try {
            ...
            res.status(403).send('403: You dont have permissions to view all repos, or your token has expired.');
        }
    }
    ...
    async getOneRepo(req, res) {
        try {
            ...
            if (repo) {
                ...
                return res.status(404).send('404: Repo is not found.');
            }
        } catch (err) {
            console.log(err);
            res.status(403).send('403: You dont have permissions to view this repo, or your token has expired.');
        }
    }
    ...
    async createRepo(req, res) {
        try {
            ...
            if (repoExists)
                return res.status(409).send('409: Repo with such name already exists.');
            ...
            console.log(err);
            res.status(403).send('403: You dont have permissions to create repo, or your token has expired.');
        }
    }
    ...
    async updateRepo(req, res) {
        try {
            ...
            if (repoWithSameName && repoWithSameName.id != req.params.id)
                return res.status(409).send('409: You already have a repo with such name.');
            if (repo) {
                await Repos.update({
                    name: req.body.name,
                }, {
                ...
                res.status(404).send('404: Repo is not found.');
        } catch (err) {
            console.log(err);
            res.status(403).send('403: You dont have permissions to update this repo, or your token has expired.');
        }
    }
    ...
    async deleteRepo(req, res) {
        try {
            ...
            if (repo) {
                ...
                return res.status(200).end(JSON.stringify(repo, null, 4));
            } else res.status(404).send('404: There is no repo with such id.');
        } catch (err) {
            console.log(err);
            res.status(403).send('403: You dont have permissions to delete this repo, or your token has expired.');
        }
    }
}
